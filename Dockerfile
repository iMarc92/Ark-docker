...
ENV GID=1000
# Ark-Server-Tools Branch (Tag/Branch on github for ark server tool)
ENV BRANCH="master"

# Install dependencies 
RUN apt-get update -y &&\ 
	apt-get install sudo bsdmainutils util-linux binutils bc jq tmux netcat -y &&\ 
	sudo dpkg --add-architecture i386 &&\
	apt-get update -y &&\
    apt-get install -y >=coreutils-7.6 sed rsync perl-modules curl lsof libc6-i386 lib32gcc1 bzip2 git
